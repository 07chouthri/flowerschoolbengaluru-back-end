...
export class DatabaseStorage implements IStorage {
  ...
  async getProductsByCategoryAndSubcategory(category: string, subcategory?: string): Promise<Product[]> {
    try {
      // If neither category nor subcategory provided, return all in-stock products
      ...
      if (category) {
        ...
        categoryClauses.push(`category ILIKE $${params.length}`);
      }
      ...
      // Build WHERE clause: require inStock and match either category or subcategory
      let where = `"inStock" = true`;
      if (categoryClauses.length > 0) {
        where += ` AND (${categoryClauses.join(' OR ')})`;
      }

      const query = `SELECT * FROM bouquetbar.products WHERE ${where} AND isactive = true ORDER BY createdat DESC`;
      console.log('Executing parameterized query:', query, 'params:', params);
      const res = await db.query(query, params);
      console.log('Query Result:', res.rows?.length ?? 0);

      // If no matching products found, fall back to returning all in-stock products (preserve previous behavior)
      if (!res.rows || res.rows.length === 0) {
        const fallbackQ = `SELECT * FROM bouquetbar.products WHERE "inStock" = true AND isactive = true ORDER BY createdat DESC`;
        console.log('No category match; executing fallback query:', fallbackQ);
        const fallbackRes = await db.query(fallbackQ);
        console.log('Fallback Query Result:', fallbackRes.rows?.length ?? 0);
  ...
  async deleteImpact(id: string): Promise<void> {
    try {
      const q = { text: `DELETE FROM bouquetbar.ourimpact
