...
export async function registerRoutes(app) {
    ...
    app.get("/api/products", async (req, res) => {
        try {
            ...
            if (category) {
                // If category is specified, use DatabaseStorage method
                const products = await storage.getProductsByCategoryAndSubcategory(category.toString(), subcategory?.toString());
    ...
    app.post("/api/admin/products", async (req, res) => {
        try {
            ...
            const productData = {
                ...
                // Normalize category: frontend may send array or string. Store as JSON string when array.
    ...
    app.post("/api/admin/products", async (req, res) => {
        try {
            ...
            console.log(`Received ${JSON.parse(req.body.stockquantity)} images`);
            const productData = {
                name: req.body.name,
                description: req.body.description,
                price: parseFloat(req.body.price),
                // Normalize category: accept array or string from client; persist as JSON string for arrays
                category: Array.isArray(req.body.category) ? JSON.stringify(req.body.category) : req.body.category,
                stockQuantity: parseInt(req.body.stockquantity),
                inStock: true,
                featured: req.body.featured || false,
                image: cleanBase64(images[0]),
                imagefirst: cleanBase64(images[1]),
                imagesecond: cleanBase64(images[2]),
                imagethirder: cleanBase64(images[3])
            };
            // Validate required fields
            if (!productData.name || !productData.description || !productData.price || !productData.category) {
                return res.status(400).json({ error: "Missing required fields" });
